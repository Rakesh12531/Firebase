...
function fetchSensorData() {
    ...
    onValue(sensorRef, (snapshot) => {
        ...
        if (data.temperature > 35) {
            alert('Warning: Temperature exceeds 35°C!');
        }
        if (data.gas > 300) {
            alert('Warning: Gas level exceeds 300!');
        }
    });
}
...
function fetchLEDStates() {
    ...
    ledPaths.forEach((path, index) => {
        ...
        onValue(ledRef, (snapshot) => {
            ...
            ledButton.className = ledState ? "ON" : "OFF";
        });
    });
}
...
window.toggleLED = function(led) {
    ...
    get(ledPath).then((snapshot) => {
        ...
        set(ledPath, currentValue === 0 ? 1 : 0).then(() => {
            ...
            document.getElementById(led).className = currentValue === 0 ? "ON" : "OFF";
        });
    });
};
